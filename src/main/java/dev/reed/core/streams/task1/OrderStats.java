...
import dev.reed.core.streams.task1.entity.OrderItem;
...
public class OrderStats {
     ...
     * referring to these sizes. Order size here is just a total number of products in the order.
     *
    ...
    public static Map<Integer, List<Order>> orderSizes(final Stream<Order> orders) {
        return orders.collect(
                Collectors.groupingBy(o -> o.getOrderItems()
                        .stream()
    ...
    public static Boolean hasColorProduct(final Stream<Order> orders, final Product.Color color) {
        return orders
                .allMatch(order -> order.getOrderItems()
                        .stream()
                        .anyMatch(item -> item.getProduct().getColor().equals(color)));
    }
